...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["p"],
  description: `Ecoute de la musique depuis YouTube ou Soundcloud`,
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Usage: ${message.client.prefix}play <YouTube URL | Nom Vidéo | Soundcloud URL>`)
        .catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT")) return message.reply(`Impossible de se connecter au canal vocal, autorisations manquantes`);
    if (!permissions.has("SPEAK")) return message.reply(`Je ne peux pas parler dans ce canal vocal, assurez-vous que j'ai les autorisations appropriées !`);
